...
<body>
	...
	<div id="wrap">
		<form name="payfieldform" method="post" action="/insertClassPay_ok"
			...
			<div id="order_wrap">
				...
				<div class="orderwrap02">
					<div class="orderwrap02-01">
						...
						<div class="servicePeriod">라운딩 횟수</div>
					</div>
					<div class="orderwrap02-02">
						<div class="serviceImage">
							<img src="/upload/class${item.fimage}" class="serviceImageValue" />
						</div>
						<div class="serviceDetail">
							<div class="serviceTitleValue">${item.ftitle}</div>
							<div class="serviceDescValue">${item.fsdesc}</div>
							<div class="serviceLevelValue">1회당 레슨 시간(분) :
								${item.fstime} 분</div>
						</div>
						<div class="serviceChargeValue">
							<c:if test="${item.fsprice != '0'}">
								<fmt:formatNumber value="${item.fsprice}" pattern="#,###" />
							</c:if>
							원
						</div>
						<div class="serviceTeacherValue">${item.tname} 프로</div>
						<div class="servicePeriodValue">${item.fsrounding} 회</div>
					</div>
				</div>
