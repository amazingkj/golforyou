...
<body>
	<div id="wrap">
		<form method="get" action="/admin/admin_classField">
			<div>
				<h1 id="name-tag">필드 클래스 관리</h1>
			</div>
			<hr class="hr-line">
			<div class="category">
				<ul class="province_1">
			...
			<br/>
			<div class="subject">
				<h1 class="cList_title">필드 클래스 전체 목록</h1>
				<br />
				<div class="cList_count">총 필드 클래스 수: ${listcount} 개</div>
				...
				<c:if test="${!empty flist}">
					<c:forEach var="f" items="${flist}">
						<tr>
							...
							{location='admin_field_del?fno=${f.fno}&page=${page}';}else{ return ;}" /></td>
						</tr>
					</c:forEach>
				</c:if>
				<c:if test="${empty flist}">
   ...
   [이전]&nbsp;
						...
						<c:if test="${page >1}">
							<a href="admin_classField?page=${page-1}">[이전]</a>&nbsp;
   </c:if>
						...
						<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
							...
							<c:if test="${a != page}">
								<a href="admin_classField?page=${a}">[${a}]</a>&nbsp;
    </c:if>
						</c:forEach>

						<c:if test="${page>=maxpage}">[다음]</c:if>
						<c:if test="${page<maxpage}">
							<a href="admin_classField?page=${page+1}">[다음]</a>
						</c:if>
					</c:if>
   ...
   [이전]&nbsp;
						...
						<c:if test="${page >1}">
							<a
								href="admin_classField?page=${page-1}&find_field=${find_field}&find_name=${find_name}">[이전]</a>&nbsp;
   </c:if>
						...
						<c:forEach var="a" begin="${startpage}" end="${endpage}" step="1">
							...
							<c:if test="${a != page}">
								<%--현재 쪽번호가 선택 안된 경우 --%>
								<a
									href="admin_classField?page=${a}&find_field=${find_field}&find_name=${find_name}">[${a}]</a>&nbsp;
    </c:if>
						</c:forEach>
						...
						<c:if test="${page<maxpage}">
							<a
								href="admin_classField?page=${page+1}&find_field=${find_field}&find_name=${find_name}">[다음]</a>
						</c:if>
					</c:if>
				</div>
				<br/>
				<div id="cFind_wrap">
					<select name="find_field" class="class-title">
						<option value="ftitle"
							<c:if test="${find_field=='ftitle'}">${'selected'}</c:if>>클래스명</option>
						<option value="tname"
						...
						value="전체목록" onclick="location='/admin/admin_classField?page=${page}';" />
