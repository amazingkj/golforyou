...
function Change(idx){
    ...
    if(nowPaging == 10){
        location.href="./board_list?pageIndex="+pagenum+"&search_page="+nowPaging;    
...
<!-- 게시판 하단 목록 검색 쓰기 버튼-->
	...
	<div id="bList_paging" class="page_control">
     ...
     <c:if test="${page>1}">
      <a href="board_list?page=${page-1}"><i class="fas fa-angle-left"></i></a>
     </c:if>
     <%--현재 쪽번호 출력 --%>
     ...
     <c:if test="${page >= maxpage}">
      ...
      <c:if test="${page < maxpage}">
      <a href="board_list?page=${page+1}"><i class="fas fa-angle-right"></i></a>
      </c:if>
    </c:if>
    ...
    <c:if test="${(!empty find_field) || (!empty find_name)}"> 
     ...
     <c:if test="${page>1}">
      <a href="board_list?page=${page-1}&find_field=${find_field}&find_name=${find_name}"><i class="fas fa-angle-left"></i></a>
     ...
     <c:if test="${page >= maxpage}">
      ...
      <c:if test="${page < maxpage}">
      <a href="board_list?page=${page+1}&find_field=${find_field}&find_name=${find_name}"><i class="fas fa-angle-right"></i></a>
      </c:if>
    </c:if>    
   </div>
</div>
