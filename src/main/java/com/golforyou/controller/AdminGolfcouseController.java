...
public class AdminGolfcouseController {
	...
	public String insertGolfcouse_ok(GolfcouseVO gc, MultipartHttpServletRequest request,Model model, MultipartFile file,HttpSession session) throws Exception {

		String saveFolder = request.getRealPath("/upload/golfcouse");
	...
	public String admin_golfcouseDetail() throws Exception {
		return "admin/admin_golfcouseDetail";
	}
	
	//회원 페이지 골프장 상세 페이지
		@RequestMapping(value="/golfcouse_Mian",method=RequestMethod.GET)
		public String golfcouse_Mian() throws Exception {
			return "golfcouse/golfcouse_Mian";
		}
	...
	public String editGolfcouse_ok(GolfcouseVO gc, HttpServletRequest request ,MultipartFile file ,HttpSession session) throws Exception {
		...
		//골프장 정보
		String gc_title=multi.getParameter("gc_title"); //골프장명
		String gc_english=multi.getParameter("gc_english"); //골프장 영문명
		String gc_area=multi.getParameter("gc_area"); //지역
		int gc_hole=Integer.parseInt(multi.getParameter("gc_hole")); //홀
		int gc_golf=Integer.parseInt(multi.getParameter("gc_golf")); //파
		int gc_length=Integer.parseInt(multi.getParameter("gc_length")); //길이
		String gc_kind=multi.getParameter("gc_kind"); //잔디종류
		String gc_type=multi.getParameter("gc_type"); //코스타입
		String gc_configuration=multi.getParameter("gc_configuration"); //코스구성
		String gc_caddy=multi.getParameter("gc_caddy");//캐디
		String gc_cart=multi.getParameter("gc_cart");//카트
		String gc_architects=multi.getParameter("gc_architects");//설계자명
		String gc_content=multi.getParameter("gc_content");//골프장내용
		String gc_coordinates=multi.getParameter("gc_coordinates");//지도 좌표값
		String gc_address_postcode=multi.getParameter("gc_address_postcode"); //우편번호
		String gc_address_roadAddress=multi.getParameter("gc_address_roadAddress"); //도로명주소
		String gc_address_jibunAddress=multi.getParameter("gc_address_jibunAddress"); //지번주소
		String gc_address_detailAddress=multi.getParameter("gc_address_detailAddress"); //상세주소
		String gc_address_extraAddress=multi.getParameter("gc_address_extraAddress"); //참고항목
		String gc_phone=multi.getParameter("gc_phone");//전화번호
		String gc_fax=multi.getParameter("gc_fax");//팩스
		String gc_move=multi.getParameter("gc_move");//골프장까지 자차이동경로
		String gc_date=multi.getParameter("gc_date");//골프장 개장날짜
		
		
		//이미지 업로드
		File upFile = multi.getFile("gc_image"); //이미지
	...
	//관리자 페이지 골프장 삭제
		...
		//좌표값확인 팝업페이지
		@RequestMapping("/admin_gcarchitects")
		public String admin_gcarchitects() throws Exception{
			return "admin/admin_gcarchitects";
		}//admin/admin_gcarchitects
