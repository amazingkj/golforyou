...
select * from cteacher c inner join online_class o on c.tno=o.tno order by odate desc

select * from cteacher c inner join field_class f on c.tno=f.tno order by fdate desc
...
drop table field_class;

drop sequence fd_seq;
...
select rowNum,c.tno,tname,tgender,tcareer,
ono,otitle,ophone,oimage,odate,oprice,odesc,otime,olevel
from cteacher c inner join online_class o
on c.tno=o.tno order by odate desc;

select * from (
		select rowNum,c.tno,tname,tgender,tcareer,
         ono,otitle,ophone,oimage,odate,oprice,odesc,otime,olevel
		from (select * from cteacher c inner join online_class o on c.tno=o.tno		
		order by odate desc));
...
select * from (
        ...
        where ftitle like '%필드%'
        order by fdate desc)) where rNum >= 1 and rNum <= 2 ;
